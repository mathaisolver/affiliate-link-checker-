...
function TopNav({
  ...
  auth: { isAuthenticated: boolean; isPro: boolean; email: string | null; name: string | null; tier: string }
  ...
  const displayName = auth.name || (auth.email ? auth.email.split("@")[0].charAt(0).toUpperCase() + auth.email.split("@")[0].slice(1) : "User")
  // Avatar initial: prefer first letter of name, fall back to first letter of email
  const avatarInitial = (auth.name?.[0] || auth.email?.[0] || "?").toUpperCase()
                  ...
                  {menuOpen && (
                    <>
                      ...
                      >
                        ...
                        <button
                          onClick={() => { setConfirmSignOut(true); setMenuOpen(false) }}
